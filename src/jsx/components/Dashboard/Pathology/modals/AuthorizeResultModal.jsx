...
const AuthorizeResultModal = ({ show, onHide, orderData, onSaveResults, onAuthorize }) => {
    ...
    if (orderData?.reportTypeGroups) {
      Object.values(orderData.reportTypeGroups).forEach(group => {
        group.parameters?.forEach(param => {
          // Initialize all parameters (both with and without results)
          initialValues[param._id] = param.resultValue || '';
        });
      });
    }
  ...
  const validateParameter = (param, value) => {
    ...
    // ALL parameters are required (same as ResultEntryModal)
  ...
  // Handle save results (edit functionality)
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      // Prepare data grouped by reportType (same format as ResultEntryModal)
        ...
        Object.entries(orderData.reportTypeGroups).forEach(([reportType, group]) => {
          ...
          group.parameters?.forEach(param => {
            ...
            reportResults.push({
              ...
              parameterValue: resultValues[param._id] || '',
            });
          });
  ...
  // Handle authorize all results
  const handleAuthorize = async () => {
    // Validate all parameters before authorization
    if (!validateAllParameters()) {
      toast.error('Please fix the validation errors before authorizing');
      return;
    }
    ...
    try {
      ...
      // Count total parameters for success message
      let totalParameters = 0;
      if (orderData?.reportTypeGroups) {
        Object.values(orderData.reportTypeGroups).forEach(group => {
          totalParameters += group.parameters?.length || 0;
        });
      }
      ...
      toast.success(`Authorized ${totalParameters} parameters successfully`);
      onHide();
    } catch (error) {
      console.error('Error authorizing results:', error);
      toast.error('Failed to authorize results');
    } finally {
  ...
  return (
    <Modal show={show} onHide={onHide} size="xl" centered backdrop="static">
      <Modal.Header closeButton>
        <Modal.Title>
          Authorize Results - {orderData?.serviceName}
      ...
      <Modal.Body>
        ...
        {orderData?.patient && (
          <div className="mb-4 p-3 bg-light rounded">
            ...
            <Row className="mt-2">
              ...
              <Col md={6}>
                <strong>Total Results:</strong> {orderData.totalResults || 0}
              </Col>
            </Row>
          </div>
        )}
        ...
        {showValidationSummary && Object.keys(validationErrors).length > 0 && (
          <div className="alert alert-danger mb-4">
            ...
            <p className="mb-0">Please fix the following issues:</p>
                ...
                <Card.Header className="bg-light text-white">
                  <h5 className="mb-0">
                    ...
                    <Badge bg="light" text="dark" className="ms-2">
                      {group.parameterCount || group.parameters?.length || 0} parameters
                    </Badge>
                  </h5>
                </Card.Header>
                            ...
                            <tr key={param._id} className={hasError ? 'table-danger' : ''}>
                              ...
                              <td>
                                <small className="text-success fw-bold">
                                  {param.bioReference?.length > 0
                                    ? param.bioReference.join(', ')
                                    : '-'}
                                </small>
                              </td>
                              <td>
        ...
        {/* No results message */}
        {(!orderData?.reportTypeGroups || allParameters.length === 0) && (
          <div className="text-center text-muted py-5">
            <i className="fas fa-clipboard-check fa-3x mb-3"></i>
            <h5>No Parameters Found</h5>
            <p>No test parameters available for authorization</p>
          </div>
        )}
      </Modal.Body>
        ...
        <div className="d-flex justify-content-between align-items-center w-100">
          <div className="text-muted">
            <small>
              ...
              Total Parameters: {allParameters.length}
            </small>
          </div>
          <div className="d-flex gap-3">
            ...
            >
              {authorizing ? (
                <>
                  <i className="fas fa-spinner fa-spin me-2"></i>Authorizing...
                </>
              ) : (
                <>
                  <i className="fas fa-check-circle me-2"></i>Authorize All
                </>
              )}
            </Button>
          </div>
