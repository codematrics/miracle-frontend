...
export const patientSchema = yup.object().shape({
  patientName: yup
    ...
    .matches(/^[a-zA-Z\s\.]+$/, 'Patient name should only contain letters, spaces, and dots'),

  relation: yup
    .mixed()
    .oneOf(['S/O', 'W/O', 'D/O', 'Other'], 'Relation must be one of: S/O, W/O, D/O, Other')
  ...
  ageUnit: yup
    .mixed()
    .oneOf(['Year', 'Month', 'Day'], 'Age unit must be one of: Year, Month, Day')
  ...
  religion: yup
    .mixed()
    .oneOf(
      ['Hindu', 'Buddhist', 'Christian', 'Jain', 'Muslim', 'Parsi', 'Sikh', 'Other', ''],
      'Invalid religion'
    )
    .nullable(),
  ...
  address: yup.object().shape({
    ...
    state: yup
      ...
      .matches(/^[a-zA-Z\s\.]+$/, 'State should only contain letters, spaces, and dots'),

    district: yup
      .string()
      .required('District is required')
      .max(50, 'District too long')
      .matches(/^[a-zA-Z\s\.]+$/, 'District should only contain letters, spaces, and dots'),

    tehsil: yup
      .string()
      .required('Tehsil is required')
      .max(50, 'Tehsil too long')
      .matches(/^[a-zA-Z\s\.]+$/, 'Tehsil should only contain letters, spaces, and dots'),

    postOffice: yup
      .string()
      .required('Post office is required')
      .max(50, 'Post office too long')
      .matches(/^[a-zA-Z\s\.]+$/, 'Post office should only contain letters, spaces, and dots'),
